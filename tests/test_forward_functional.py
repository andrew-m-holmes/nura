...
def test_add_tangent_matrix():
    ...
    a_tensor = deepnet.tensor(a, usegrad=True).mutated(grad=deepnet.tensor(np.ones((3, 3))))
    b_tensor = deepnet.tensor(b, usegrad=True).mutated(grad=deepnet.tensor(np.ones((3, 3))))
    with deepnet.autograd(enabled=True, rev=False):
        result_tensor = f.add(a_tensor, b_tensor)
...
def test_matmul_tangent_different_shapes():
    ...
    a_tensor = deepnet.tensor(a, usegrad=True).mutated(grad=deepnet.tensor(np.ones((4, 3))))
    b_tensor = deepnet.tensor(b, usegrad=True).mutated(grad=deepnet.tensor(np.ones((3, 5))))
    with deepnet.autograd(enabled=True, rev=False):
        result_tensor = f.matmul(a_tensor, b_tensor)
...
def test_matmul_tangent_higher_rank_same_shape():
    ...
    a_tensor = deepnet.tensor(a, usegrad=True).mutated(grad=deepnet.tensor(np.ones((2, 3, 4, 4))))
    b_tensor = deepnet.tensor(b, usegrad=True).mutated(grad=deepnet.tensor(np.ones((2, 3, 4, 4))))
    with deepnet.autograd(enabled=True, rev=False):
        result_tensor = f.matmul(a_tensor, b_tensor)
...
def test_matmul_tangent_higher_rank_different_shape():
    ...
    a_tensor = deepnet.tensor(a, usegrad=True).mutated(grad=deepnet.tensor(np.ones((2, 4, 3))))
    b_tensor = deepnet.tensor(b, usegrad=True).mutated(grad=deepnet.tensor(np.ones((2, 3, 5))))
    with deepnet.autograd(enabled=True, rev=False):
        result_tensor = f.matmul(a_tensor, b_tensor)
...
def test_exp_tangent_matrix():
    ...
    a_tensor = deepnet.tensor(a, usegrad=True).mutated(grad=deepnet.tensor(np.ones((3, 4))))
    with deepnet.autograd(enabled=True, rev=False):
        result_tensor = f.exp(a_tensor)
...
def test_cos_tangent_matrix():
    ...
    a_tensor = deepnet.tensor(a, usegrad=True).mutated(grad=deepnet.tensor(np.ones((4, 4))))
    with deepnet.autograd(enabled=True, rev=False):
        result_tensor = f.cos(a_tensor)
    h = 1e-8
    expected_tangent = (np.cos(a + h) - np.cos(a - h)) / (2 * h)
...
def test_sum_tangent_multiple_dim():
    ...
    a_tensor = deepnet.tensor(a, usegrad=True).mutated(grad=deepnet.tensor(np.ones((3, 4, 5))))
    with deepnet.autograd(enabled=True, rev=False):
...
def test_sum_tangent_higher_rank_tensor():
    ...
    a_tensor = deepnet.tensor(a, usegrad=True).mutated(grad=deepnet.tensor(np.ones((2, 3, 4, 5))))
    with deepnet.autograd(enabled=True, rev=False):
...
def test_squeeze_tangent_rank2_v0():
    ...
    a_tensor = deepnet.tensor(a, usegrad=True).mutated(grad=deepnet.tensor(np.ones((5, 5))))
    with deepnet.autograd(enabled=True, rev=False):
        result_tensor = deepnet.squeeze(a_tensor)
...
def test_squeeze_tangent_rank2_v1():
    ...
    a_tensor = deepnet.tensor(a, usegrad=True).mutated(grad=deepnet.tensor(np.ones((3, 1))))
    with deepnet.autograd(enabled=True, rev=False):
        result_tensor = deepnet.squeeze(a_tensor)
...
def test_squeeze_tangent_multi_v0():
    ...
    a_tensor = deepnet.tensor(a, usegrad=True).mutated(grad=deepnet.tensor(np.ones((2, 1, 4, 1, 5))))
    with deepnet.autograd(enabled=True, rev=False):
        result_tensor = deepnet.squeeze(a_tensor)
...
def test_squeeze_tangent_multi_v2():
    ...
    a_tensor = deepnet.tensor(a, usegrad=True).mutated(grad=deepnet.tensor(np.ones((3, 3, 1, 7))))
    with deepnet.autograd(enabled=True, rev=False):
        result_tensor = deepnet.squeeze(a_tensor)
...
def test_unsqueeze_tangent_multi_v1():
    ...
    a_tensor = deepnet.tensor(a, usegrad=True).mutated(grad=deepnet.tensor(np.ones((2, 3))))
    with deepnet.autograd(enabled=True, rev=False):
...
def test_unsqueeze_tangent_multi_v2():
    ...
    a_tensor = deepnet.tensor(a, usegrad=True).mutated(grad=deepnet.tensor(np.ones((5, 6, 7, 8))))
    with deepnet.autograd(enabled=True, rev=False):
...
def test_unsqueeze_tangent_multi_v4():
    ...
    a_tensor = deepnet.tensor(a, usegrad=True).mutated(grad=deepnet.tensor(np.ones((2, 5, 3))))
    with deepnet.autograd(enabled=True, rev=False):
...
def test_permute_tangent_rank2_v0():
    ...
    a_tensor = deepnet.tensor(a, usegrad=True).mutated(grad=deepnet.tensor(np.ones((8, 15))))
    with deepnet.autograd(enabled=True, rev=False):
...
def test_permute_tangent_rank3_v0():
    ...
    a_tensor = deepnet.tensor(a, usegrad=True).mutated(grad=deepnet.tensor(np.ones((4, 5, 6))))
    with deepnet.autograd(enabled=True, rev=False):
...
def test_permute_tangent_rank3_v1():
    ...
    a_tensor = deepnet.tensor(a, usegrad=True).mutated(grad=deepnet.tensor(np.ones((70, 15, 512))))
    with deepnet.autograd(enabled=True, rev=False):
...
def test_permute_tangent_rank4_v0():
    ...
    a_tensor = deepnet.tensor(a, usegrad=True).mutated(grad=deepnet.tensor(np.ones((3, 4, 5, 6))))
    with deepnet.autograd(enabled=True, rev=False):
...
def test_permute_tangent_rank4_v1():
    ...
    a_tensor = deepnet.tensor(a, usegrad=True).mutated(grad=deepnet.tensor(np.ones((6, 7, 8, 9))))
    with deepnet.autograd(enabled=True, rev=False):
...
def test_view_tangent_rank2_to_rank3():
    ...
    a_tensor = deepnet.tensor(a, usegrad=True).mutated(grad=deepnet.tensor(np.ones((6, 4))))
    with deepnet.autograd(enabled=True, rev=False):
...
def test_view_tangent_rank3_to_rank4():
    ...
    a_tensor = deepnet.tensor(a, usegrad=True).mutated(grad=deepnet.tensor(np.ones((2, 3, 4))))
    with deepnet.autograd(enabled=True, rev=False):
...
def test_view_tangent_rank4_to_rank2():
    ...
    a_tensor = deepnet.tensor(a, usegrad=True).mutated(grad=deepnet.tensor(np.ones((2, 2, 3, 2))))
    with deepnet.autograd(enabled=True, rev=False):
...
def test_reshape_tangent_rank2_to_rank3():
    ...
    a_tensor = deepnet.tensor(a, usegrad=True).mutated(grad=deepnet.tensor(np.ones((8, 6))))
    with deepnet.autograd(enabled=True, rev=False):
...
def test_reshape_tangent_rank3_to_rank4():
    ...
    a_tensor = deepnet.tensor(a, usegrad=True).mutated(grad=deepnet.tensor(np.ones((3, 2, 6))))
    with deepnet.autograd(enabled=True, rev=False):
...
def test_reshape_tangent_rank2_to_rank1():
    ...
    a_tensor = deepnet.tensor(a, usegrad=True).mutated(grad=deepnet.tensor(np.ones((5, 4))))
    with deepnet.autograd(enabled=True, rev=False):
...
def test_reshape_tangent_rank3_to_rank2():
    ...
    a_tensor = deepnet.tensor(a, usegrad=True).mutated(grad=deepnet.tensor(np.ones((2, 6, 4))))
    with deepnet.autograd(enabled=True, rev=False):
...
def test_reshape_tangent_rank4_to_rank2():
    ...
    a_tensor = deepnet.tensor(a, usegrad=True).mutated(grad=deepnet.tensor(np.ones((4, 3, 2, 2))))
    with deepnet.autograd(enabled=True, rev=False):
...
def test_reshape_tangent_with_negative_dim():
    ...
    a_tensor = deepnet.tensor(a, usegrad=True).mutated(grad=deepnet.tensor(np.ones((4, 5, 3))))
    with deepnet.autograd(enabled=True, rev=False):
